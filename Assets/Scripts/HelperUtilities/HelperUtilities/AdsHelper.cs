...
namespace UnityHelperSDK.HelperUtilities{
    ...
    public class AdsHelper : MonoBehaviour
    {
        ...
        private async Task ResetDailyCountsIfNeeded()
        {
            ...
            {
                await PrefsHelper.Set(GamePrefs.InterstitialCount, 0); // Reset interstitial count
                await PrefsHelper.Set(GamePrefs.RewardedAdCount, 0); // Reset rewarded count
                await PrefsHelper.SetJson(GamePrefs.AdsDateTimeComplexData, DateTime.Today);
            }
        }
        ...
        private async Task TrackInterstitialShown()
        {
            if (!enableAdTracking) return;

            var count = PrefsHelper.Get<int, GamePrefs>(GamePrefs.InterstitialCount, 0);
            await PrefsHelper.Set(GamePrefs.InterstitialCount, count + 1);
            await PrefsHelper.SetJson(GamePrefs.AdsDateTimeComplexData, DateTime.Now);
        }

        private async Task TrackRewardedShown()
        {
            if (!enableAdTracking) return;

            var count = PrefsHelper.Get<int, GamePrefs>(GamePrefs.RewardedAdCount, 0);
            await PrefsHelper.Set(GamePrefs.RewardedAdCount, count + 1);
            await PrefsHelper.SetJson(GamePrefs.AdsDateTimeComplexData, DateTime.Now);
        }
